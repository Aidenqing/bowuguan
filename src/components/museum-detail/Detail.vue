...
<div class="exhibits">
  ...
  <div class="more-data">
    ...
    <div class="zan">
      ...
      <span>{{exhibits[currentIndex].collectCount}}</span>
    </div>
  </div>
  <div class="info">
    <div class="name">{{exhibits[currentIndex].exhibitsName}}</div>
    <div class="intro">{{exhibits[currentIndex].exhibitsIntroduce}}</div>
  </div>
  <div class="bottom">
    <div class="btn" @click="toMuseum">所有展品</div>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    _getExhibits () {
      ...
      // this.InitTabScroll()
      getMuseumDetail(this.museumId).then((res) => {
    ...
    _handleOriginData (data) {
      this.setTitle(data.museum.museumName)

    this._getCurrentIndex(data.museum.exhibits)
    this.exhibits=data.museum.exhibits
    },
    _getCurrentIndex(exhibits){
      if(!exhibits){
        exhibits=this.exhibits
      }
      let len=exhibits.length
      let item=[]
      for (let i=0;i<len;i++) {
        item=exhibits[i]
        if (item.id == this.exhibitsId) {
          this.currentIndex=i
          break;
        }
      }
 if(this.slider){
  this.slider.slideTo(this.currentIndex+1)
      ...
      this.slider = new Swiper('.slider', {
        ...
        on: {
          slideChangeTransitionEnd: function(){
            ...
            self.currentIndex=this.activeIndex-1//切换结束时，告诉我现在是第几个slide
